...
export default {
  ...
  created() {
    ...
    this.skuId = this.$route.query.sid
    this.productCount = this.$route.query.pct ? parseInt(this.$route.query.pct) : 1
    if (this.productId != null && this.skuId != null) {
      this.getOrderInfo()
    } else {
  ...
  methods: {
    ...
    async getOrderInfo() {
      this.loading = true
      try {
        const res = await orderApi.checkDirect({ productId: this.productId, skuId: this.skuId, productCount: this.productCount })
  ...
  watch: {
    $route(to, from) {
      if (to.path === '/layout/order/checkDirect' && from.path === '/layout/personal/address') {
        this.initAddress()
      }
    },
  },
